...
`AIMemory Agent Template`은 Codex, Claude Code, Gemini CLI 같은 여러 코딩 에이전트가 같은 프로젝트 폴더에서 작업 맥락을 공유하고, 작업을 이어받을 수 있도록 돕는 파일 기반 협업 메모리 템플릿입니다.
...
- Codex, Claude, Gemini 사이에서 수동으로 설명을 복사해 붙여넣어야 함
...
| `GEMINI.md` | Gemini CLI가 읽는 프로젝트 지침 파일 |
...
그다음 Codex, Claude Code, Gemini CLI 중 하나를 프로젝트 루트에서 실행하고 다음처럼 요청합니다.
...
루트의 `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`가 `AIMemory/PROTOCOL.md`를 참조하므로, 에이전트는 작업 전후로 AIMemory를 활용하도록 안내됩니다.

## 사용 방법 2. 기존 프로젝트에 안전하게 적용하기

기존 프로젝트에는 템플릿 전체를 그대로 풀지 않는 편이 안전합니다.

기존 프로젝트에는 이미 `AGENTS.md`, `CLAUDE.md`, `GEMINI.md` 또는 다른 설정 파일이 있을 수 있기 때문입니다. 이 경우에는 zip 파일에서 `aimemory-agent-setup.md`만 꺼내 사용합니다.
...
새 프로젝트 `dice-game`에서 Codex, Claude Code, Gemini CLI를 같은 폴더로 엽니다.
...
마지막으로 Gemini CLI에서 다음처럼 요청할 수 있습니다.
...
```text
### 2026-04-28 16:20 | claude-opus-4-5 | CHECKPOINT
...
- Codex, Claude Code, Gemini CLI를 번갈아 사용하는 경우
