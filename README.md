...
에이전트는 작업 전에 `.AIMemory/`를 읽고, 작업 후 `.AIMemory/work.log`에 기록하며, 다른 에이전트가 이어받아야 할 때 `.AIMemory/handoffs/`에 인계 파일을 남깁니다.
...
2. 전체 세션 시작 루틴(.AIMemory/PROTOCOL.md, .AIMemory/PROJECT_OVERVIEW.md, .AIMemory/work.log 순 읽기)을 실행하지 않는다.
3. 답변을 보고하고 멈춘다.
...
루트의 `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `opencode.md`가 `.AIMemory/PROTOCOL.md`를 참조하므로, 에이전트는 작업 전후로 `.AIMemory/`를 활용하도록 안내됩니다.
...
```text
1. Codex가 기능 구현
2. Codex가 .AIMemory/work.log에 작업 기록
3. Codex가 Claude용 handoff 파일 생성
4. Claude가 .AIMemory와 handoff 파일을 읽고 보완
5. Claude가 .AIMemory/work.log에 결과 기록
6. Gemini가 UX 문구나 설명을 검토
7. Gemini가 .AIMemory/work.log와 필요한 handoff를 기록
```
...
4. 남은 작업 기록
5. 위험 요소와 미확정 사항 기록
6. .AIMemory/work.log에 CHECKPOINT, WORK_END 또는 HANDOFF 기록
...
최근 기록은 `.AIMemory/work.log`에서 확인하고, 오래된 내용은 `.AIMemory/INDEX.md`를 통해 필요한 파일만 찾아 읽습니다.
...
```text
AIMemory 프로토콜에 따라 이번 작업을 .AIMemory/work.log에 기록하고 handoff 파일도 만들어라.
```

`.AIMemory/` 폴더는 앞에 피리어드(`.`)를 붙인 이름을 사용합니다. 이렇게 하면 Windows 탐색기, VS Code 탐색기, macOS Finder 등에서 기본적으로 숨겨지거나 하단으로 밀려 프로젝트 파일과 시각적으로 분리됩니다. `.git/`, `.vscode/` 같은 기존 도구 관례와도 일치합니다. Windows에서는 탐색기로 직접 생성할 수 없으므로, PowerShell이나 터미널에서 생성해야 합니다.
...
다른 에이전트는 같은 폴더에서 `.AIMemory`를 읽고 이어받습니다.
