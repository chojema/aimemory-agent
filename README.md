...
`AIMemory Agent Template`은 Codex, Claude Code, Gemini CLI, OpenCode 같은 여러 코딩 에이전트가 같은 프로젝트 폴더에서 작업 맥락을 공유하고, 작업을 이어받을 수 있도록 돕는 파일 기반 협업 메모리 템플릿입니다.
...
그다음 Codex, Claude Code, Gemini CLI, OpenCode 중 하나를 프로젝트 루트에서 실행하고 다음처럼 요청합니다.
...
새 프로젝트 `dice-game`에서 Codex, Claude Code, Gemini CLI, OpenCode를 같은 폴더로 엽니다.
...
- Codex, Claude Code, Gemini CLI, OpenCode를 번갈아 사용하는 경우
