...
Your task is to install or re-engage a shared project memory system named `AIMemory`, and to wire this project so that Codex, Claude Code, Gemini CLI, OpenCode, and other file-aware coding agents naturally read and update that memory on future turns.
...
If you do not have filesystem read/write access, stop and tell the user that this setup must be run from an agentic coding environment such as Codex CLI, Claude Code, Gemini CLI, Cursor agent mode, Aider, Cline, Continue, OpenCode, or a similar tool.
...
Harness: <Claude Code | Codex CLI | Gemini CLI | Cursor | Aider | Cline | Continue | OpenCode | Windsurf | other>
